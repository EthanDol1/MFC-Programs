...
def linear_voltage():
    ...
    if start_voltage.get() == '':
        if messagebox.askyesno('Mass Flow GUI',\
            'Are you sure you want to leave the start voltage as its default setting? \nThe default setting is whatever the current voltage is.'):
            sv = v
            b1 = True
        else:
            b1 = False
...
def help():
    messagebox.showinfo('Mass Flow GUI','Here is some info on how the GUI works.\nThe output voltage from the Arduino Uno varies from 0 - 5V.\n1) The Apply Max Voltage button instructs the Arduino to output 5V.\n2) The Kill Voltage button instructs the Arduino to output 0V.\n3) The Apply Manual Change button takes the value enterred into the entry slot above the button and instructs the Arduino to output that specific voltage.\n4) The Apply Linear Change button takes in 4 entries as parameters: the start voltage, the end voltage, the step size, and the time between steps. This button has the effect of an automated gradual change in voltage depending upon the parameters you enter in.\n   a) The start voltage entry and end voltage entry instruct the Arduino on what voltages you would like to start and end at. The start voltage does not have to be lower than the end voltage, i.e. you may use this button to go up or down in voltage. Because of discrepancies between the voltages written to the Arduino and the voltages that it actually outputs, there is an error in the display voltage of approximately +/- 1 step.\n    b) The step size entry instructs the Arduino on how big of steps to take to get from the start voltage to the end voltage, e.g. with a start voltage of 0, an end voltage of 1, and a step size of 0.1, the Arduino would take 10 steps incrementing like 0.1 .. 0.2 .. 0.3 .. and so on. Additionally, if you wish to go down in voltage from a higher start voltage to a lower end voltage, you should still enter in a positive-valued step size.\n   c) The time between entry instructs the Arduino on how many seconds you would like it to wait after performing one step before it performs the next step. 20 milliseconds is added on to this time between in order for the GUI to run smoothly.')
